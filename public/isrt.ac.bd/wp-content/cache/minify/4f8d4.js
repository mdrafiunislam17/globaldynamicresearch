...
(function (root, factory) {
  ...
  var getOrigin = function getOrigin() {
    ...
    if (!node || !document.body.contains(node)) {
      ...
      extend(node.style, { top: 0, left: 0, position: "absolute" });
  ...
  var MIRROR_LR = { center: "center", left: "right", right: "left" };
  var MIRROR_TB = { middle: "middle", top: "bottom", bottom: "top" };
  var OFFSET_MAP = {
    top: 0,
    left: 0,
    middle: "50%",
...
+(function () {
  ...
  var Popover = (function ($) {
    ...
    var ClassName = { FADE: "fade", SHOW: "show" };
    var Selector = { TITLE: ".popover-title", CONTENT: ".popover-content" };
    var Event = {
      HIDE: "hide" + EVENT_KEY,
      HIDDEN: "hidden" + EVENT_KEY,
      SHOW: "show" + EVENT_KEY,
  ...
  $.maxmegamenu = function (menu, options) {
    ...
    plugin.addAnimatingClass = function (element) {
      ...
      var timeout =
        ...
        parseInt(plugin.settings.hover_intent_timeout, 10);
    ...
    plugin.expandMobileSubMenus = function () {
      ...
      if (plugin.settings.mobile_state == "expand_all") {
        $(
          ".mega-menu-item-has-children:not(.mega-toggle-on) > a.mega-menu-link",
          $menu
        ).each(function () {
    ...
    plugin.showPanel = function (anchor, immediate) {
      if ($.isNumeric(anchor)) {
        anchor = $("li.mega-menu-item-" + anchor, $menu)
    ...
    plugin.bindClickEvents = function () {
      ...
      $(document).on("click touchend", function (e) {
        if (
          ...
          !$(e.target).closest(".mega-menu-wrap").length
        ) {
    ...
    plugin.bindKeyboardEvents = function () {
      ...
      var isMobileOffCanvasHorizontal = function () {
        return (
          plugin.isMobileView() &&
          (plugin.settings.effect_mobile === "slide_right" ||
            plugin.settings.effect_mobile === "") &&
          plugin.settings.mobile_direction === "horizontal"
        );
      };
      var shouldTrapFocusInCurrentSubMenu = function () {
        return (
          isMobileOffCanvasHorizontal() &&
        ...
        function (e) {
          ...
          if (keyCode === escape_key) {
            ...
            if (submenu_open) {
              ...
              if (
                focused_menu_item.closest(".mega-menu-flyout.mega-toggle-on")
                  .length !== 0
              ) {
                var nearest_parent_of_focused_item_li =
                  focused_menu_item.closest(".mega-toggle-on");
                var nearest_parent_of_focused_item_a = $(
                  "> a.mega-menu-link",
                  nearest_parent_of_focused_item_li
                );
                plugin.hidePanel(nearest_parent_of_focused_item_a);
                nearest_parent_of_focused_item_a.focus();
              }
              if (
                focused_menu_item.closest(".mega-menu-megamenu.mega-toggle-on")
                  .length !== 0
              ) {
                var nearest_parent_of_focused_item_li =
                  focused_menu_item.closest(
                    ".mega-menu-megamenu.mega-toggle-on"
                  );
                var nearest_parent_of_focused_item_a = $(
                  "> a.mega-menu-link",
                  nearest_parent_of_focused_item_li
          ...
          if (keyCode === enter_key) {
            ...
            if (active_link.parent().is(items_with_submenus)) {
              if (
                plugin.isMobileView() &&
                active_link.parent().is(".mega-hide-sub-menu-on-mobile")
              ) {
                return;
              }
              if (
          ...
          if (shouldTrapFocusInCurrentSubMenu()) {
            ...
            if (focused_item.length === 0) {
              e.preventDefault();
              $("> li.mega-menu-item:visible", $menu)
                .find("> a.mega-menu-link, .mega-search span[role=button]")
                .first()
                .focus();
              return;
            }
            var next_item_to_focus = focused_item
              .parent()
              .nextAll("li.mega-menu-item:visible")
              .find("> a.mega-menu-link, .mega-search span[role=button]")
              .first();
            if (
              next_item_to_focus.length === 0 &&
              focused_item.closest(".mega-menu-megamenu").length !== 0
            ) {
              var all_li_parents = focused_item.parentsUntil(
                ".mega-menu-megamenu"
              );
              if (
                focused_item.is(all_li_parents.find("a.mega-menu-link").last())
              ) {
                next_item_to_focus = all_li_parents
                  .find(".mega-back-button:visible > a.mega-menu-link")
                  .first();
              }
            }
            if (next_item_to_focus.length === 0) {
              next_item_to_focus = focused_item
                .parent()
